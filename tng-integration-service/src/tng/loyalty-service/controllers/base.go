...
import (
	...
	dtos2 "tng/loyalty-service/dtos"

	"tng/common/httpcode"
	"tng/common/logger"
	"tng/common/utils/cfgutil"
	"tng/loyalty-service/services"
	"tng/loyalty-service/validation"
...
type BaseController struct {
	...
	maintainConfigurationService services.MaintainConfigurationService
}
...
func (c *BaseController) Prepare() {
	c.init()
	if c.isMaintaining() {
		c.Respond(nil, dtos2.NewAppError(dtos2.ServiceUnavailableError))
		return
	}
}
...
func (c *BaseController) init() {
	...
	_ = services.GetServiceContainer().Invoke(func(s services.MaintainConfigurationService) {
		c.maintainConfigurationService = s
	})
...
func init() {
	...
	_ = globalValidator.RegisterValidation("isoDateTime", validation.Iso8601DateTime)
	_ = globalValidator.RegisterValidation("userNamePattern", validation.UserNamePattern)
	_ = globalValidator.RegisterValidation("password", validation.ValidatePassword)
	_ = globalValidator.RegisterValidation("logo", validation.ValidLogoImage)
	_ = globalValidator.RegisterValidation("phoneNumber", validation.ValidPhoneNumber)
}
...
func (c *BaseController) isMaintaining() bool {
	module, action := c.GetModuleAndMethodName()
	return c.maintainConfigurationService.IsDisable(c.Ctx.Request.Context(), module, action)
}

func (c *BaseController) CheckMeta(meta *dtos2.MetaRequest) bool {
	if meta.CheckSum == "" {
		return false
	}
	if meta.ClientIP == "" {
	...
	clientKey := cfgutil.Load("CLIENT_KEY")
	dataInput := fmt.Sprintf("%s|%d|%s|%s|%s|%s", meta.MC, meta.TS, meta.UserAgent, meta.ClientIP, meta.Data, clientKey)
	checksum := hashutil.GetSHA256(dataInput)
