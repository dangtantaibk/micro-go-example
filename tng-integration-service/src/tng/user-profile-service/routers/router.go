...
import (
	...
	"tng/user-profile-service/controllers"
...
func init() {
	...
	ns := beego.NewNamespace("/tng/api/v1",
		beego.NSNamespace("/user",
			beego.NSInclude(&controllers.UserProfileController{}),
		),
		beego.NSNamespace("/authentication",
			beego.NSInclude(&controllers.AuthenticationController{}),
		),
	)
...
func initialDB() {
	...
	orm.RegisterModel(new(userProfile.User))
}
