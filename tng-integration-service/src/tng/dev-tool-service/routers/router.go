...
import (
	...
	"tng/dev-tool-service/controllers"
...
func init() {
	...
	ns := beego.NewNamespace("/tng/api/v1",
		beego.NSNamespace("/dev-tool",
			beego.NSInclude(&controllers.Pi3UpdateInfoController{},
			),
		),
	)
...
func initialDB() {
	...
	orm.RegisterModel(new(h5_intergration.H5ZaloPay))
	orm.RegisterModel(new(dev_tool.DeviceUpdateInfo))
	orm.RegisterModel(new(models.MaintainConfiguration))
}
